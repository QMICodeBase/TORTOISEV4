...
vtkStandardNewMacro(vtkTensorGlyphOkan);

// Construct object with scaling on and scale factor 1.0. Eigenvalues are 
// extracted, glyphs are colored with input scalar data, and logarithmic
// scaling is turned off.
vtkTensorGlyphOkan::vtkTensorGlyphOkan()
{
  this->Scaling = 1;
...
//----------------------------------------------------------------------------
int vtkTensorGlyphOkan::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
int vtkTensorGlyphOkan::RequestData(
  vtkInformation *vtkNotUsed(request),
  vtkInformationVector **inputVector,
  vtkInformationVector *outputVector)
{
  ...
  for (inPtId=0; inPtId < numPts; inPtId++)
    ...
    for (dir=0; dir < numDirs; dir++)
      ...
      // old behaviour of vtkTensorGlyphOkans (which only used one dir),
      ...
      if ( this->ColorGlyphs && inScalars &&           (this->ColorMode == COLOR_BY_SCALARS) )
        {
          unsigned char sok[3];
          sok[0]= inScalars->GetComponent(inPtId, 0);
          sok[1]= inScalars->GetComponent(inPtId, 1);
          sok[2]= inScalars->GetComponent(inPtId, 2);
...
//----------------------------------------------------------------------------
void vtkTensorGlyphOkan::SetSourceConnection(int id, vtkAlgorithmOutput* algOutput)
{
  if (id < 0)
    {
...
//----------------------------------------------------------------------------
void vtkTensorGlyphOkan::SetSourceData(vtkPolyData *source)
{
  this->SetInputData(1, source);
}

//----------------------------------------------------------------------------
vtkPolyData *vtkTensorGlyphOkan::GetSource()
{
...
//----------------------------------------------------------------------------
int vtkTensorGlyphOkan::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 1)
    {
...
//----------------------------------------------------------------------------
void vtkTensorGlyphOkan::PrintSelf(ostream& os, vtkIndent indent)
{

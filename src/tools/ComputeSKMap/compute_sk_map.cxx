...
int main( int argc , char * argv[] )
{
    if(argc<2)    
    {
        std::cout<< "Computes skewness from the diffusion tensor"<<std::endl;
        std::cout<<"Usage:   ComputeSKMap full_path_to_tensor_image"<<std::endl;
        return 0;
    }
    ...
    for(int k=0;k<imsize[2];k++)
    {
        ...
        for(int j=0;j<imsize[1];j++)
        {
            ...
            for(int i=0;i<imsize[0];i++)
            {
                ...
                double lambda_1= eig.D(2,2);
                double lambda_2= eig.D(1,1);
                double lambda_3= eig.D(0,0);
                double MD= (lambda_1 + lambda_2 + lambda_3)/3.;
                double val = (MD - lambda_2)/MD;

                scalar_image->SetPixel(ind,val);
            }   
        }   
    }
    ...
    std::string output_name=currdir + basename + std::string("_SK.nii");

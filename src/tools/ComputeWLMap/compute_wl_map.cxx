...
int main( int argc , char * argv[] )
{
    if(argc<2)    
    {
        std::cout<< "Computes the Westin-Linear map from the diffusion tensor"<<std::endl;
        std::cout<<"Usage:   ComputeWLMap full_path_to_tensor_image"<<std::endl;
        return 0;
    }
    ...
    ImageType3D::Pointer scalar_image = compute_wl(image4D);
       
        
    std::string filename(argv[1]);
    std::string::size_type idx=filename.rfind('.');
    std::string basename= filename.substr(mypos+1,idx-mypos-1);
    std::string output_name=currdir + basename + std::string("_WL.nii");

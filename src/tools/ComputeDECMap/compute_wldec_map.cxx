...
#include "../ComputeWLMap/compute_wl_map.h"
...
/*
    ...
    */
    ...
    ImageType3D::Pointer wl_map= compute_wl(image4D);
    ...
    RGBImageType::Pointer WLDECimage = RGBImageType::New();
    WLDECimage->SetRegions(reg);
    WLDECimage->Allocate();
    WLDECimage->SetOrigin(norig);
    WLDECimage->SetSpacing(nspc);
    WLDECimage->SetDirection(ndir);
    RGBPixelType zero;zero.Fill(0);
    WLDECimage->FillBuffer(zero);
    ...
    for(int k=0;k<imsize[2];k++)
    {
        ...
        for(int j=0;j<imsize[1];j++)
        {
            ...
            for(int i=0;i<imsize[0];i++)
            {
                ...
                double WL= wl_map->GetPixel(ind);


                double scal= (WL-color_lattmin)/(color_lattmax-color_lattmin);
                if(scal >1)
                    scal=1;
                if(scal<0.)
                    scal=0;
    ...
    std::string basename= filename.substr(mypos+1,idx-mypos-1);
    std::string output_name=currdir + basename + std::string("_DECWL.nii");

...
__device__ void ComputeEigVecs2(Matrix3f &x, Matrix3f &eig_val, Matrix3f &eig_vec)
{
    typedef float scalar_t;
    for(int e=0;e<3;e++)
    {
        const scalar_t x11 = x(0,0) -eig_val(e,e);
        const scalar_t x12 = x(0,1);
        const scalar_t x13 = x(0,2);
        const scalar_t x21 = x(1,0);
        const scalar_t x22 = x(1,1) -eig_val(e,e);
        const scalar_t x23 = x(1,2);
        const scalar_t x31 = x(2,0);
        const scalar_t x32 = x(2,1);
        const scalar_t x33 = x(2,2) -eig_val(e,e);
...
__device__ void ComputeEigenVals2(Matrix3f &x, Matrix3f &vals)
{
    typedef float scalar_t;
    ...
    else
    {
        ...
        vals(0,0) = q + 2 * p * cos(phi);
        vals(2,2) = q + 2 * p * cos(phi + (2 * CUDART_PI_F / 3));
        vals(1,1) = 3 * q - vals(0,0) - vals(2,2);
    }
...
__device__ void ComputeEigens2(Matrix3f &mat, Matrix3f &vals, Matrix3f &vecs)
{
    ComputeEigenVals2(mat, vals);
    ComputeEigVecs2(mat, vals, vecs);
}

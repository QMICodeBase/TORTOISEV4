...
void DRTAMASStage::PreprocessImagesAndFields()
{
    ...
    if(this->settings->const_init_finv && this->settings->const_init_finv->sz.x != this->virtual_img->sz.x)
    {
        this->settings->const_init_finv = ResampleImage(this->settings->const_init_finv, this->virtual_img);
        this->settings->const_init_minv = ResampleImage(this->settings->const_init_minv, this->virtual_img);
    }
...
#include "itkVectorGradientAnisotropicDiffusionImageFilterOkan.h"
DisplacementFieldType::Pointer AnisotropicSmoothField(DisplacementFieldType::Pointer field, ImageType3D::Pointer TR_img, ImageType3D::Pointer FA_img)
{

    using ConcatenatedImagePixelType= itk::Vector<float,5>;
    using ConcatenatedImageType = itk::Image<ConcatenatedImagePixelType,3>;
    using ImageType= ImageType3D;


    DisplacementFieldType::Pointer fieldR = DisplacementFieldType::New();
    fieldR->SetRegions(field->GetLargestPossibleRegion());
    fieldR->Allocate();
    fieldR->SetSpacing(field->GetSpacing());
    fieldR->SetOrigin(field->GetOrigin());
    fieldR->SetDirection(field->GetDirection());

    double mxf=-1e10;
    double mnf=1E10;
    {

        itk::ImageRegionIterator<DisplacementFieldType> it3(field,field->GetLargestPossibleRegion());
        for(it3.GoToBegin();!it3.IsAtEnd();++it3)
        {
            DisplacementFieldType::PixelType vec= it3.Get();
            if(vec[0]> mxf)
                mxf=vec[0];
            if(vec[1]> mxf)
                mxf=vec[1];
    ...
    {
        double mx=-1e10;
        itk::ImageRegionIterator<ImageType> it3(TR_img,TR_img->GetLargestPossibleRegion());
        for(it3.GoToBegin();!it3.IsAtEnd();++it3)
            if(it3.Get()>mx)
                mx=it3.Get();

        for(it3.GoToBegin();!it3.IsAtEnd();++it3)
            it3.Set(it3.Get()/9000.*(mxf-mnf) + mnf );
    }
    {
        double mx=-1e10;
        itk::ImageRegionIterator<ImageType> it3(FA_img,FA_img->GetLargestPossibleRegion());
        for(it3.GoToBegin();!it3.IsAtEnd();++it3)
            it3.Set(it3.Get()*(mxf-mnf) + mnf);
    }



    ConcatenatedImageType::Pointer concat_img = ConcatenatedImageType::New();
    concat_img->SetRegions(field->GetLargestPossibleRegion());
    concat_img->Allocate();
    concat_img->SetSpacing(field->GetSpacing());
    concat_img->SetOrigin(field->GetOrigin());
    concat_img->SetDirection(field->GetDirection());

    itk::ImageRegionIteratorWithIndex<ConcatenatedImageType> it(concat_img,concat_img->GetLargestPossibleRegion());
    for(it.GoToBegin();!it.IsAtEnd();++it)
    {
        ConcatenatedImageType::IndexType ind3= it.GetIndex();
        ConcatenatedImageType::PixelType concat_vec;
        DisplacementFieldType::PixelType vec = field->GetPixel(ind3);
        ImageType::PixelType TR = TR_img->GetPixel(ind3);
        ImageType::PixelType FA = FA_img->GetPixel(ind3);
    ...
    // using FilterType = itk::VectorGradientAnisotropicDiffusionImageFilter<ConcatenatedImageType, ConcatenatedImageType>;
    ...
    itk::ImageRegionIteratorWithIndex<ConcatenatedImageType> it2(filter_img,filter_img->GetLargestPossibleRegion());
    for(it2.GoToBegin();!it2.IsAtEnd();++it2)
    {
        ConcatenatedImageType::IndexType ind3= it2.GetIndex();
        ConcatenatedImageType::PixelType concat_vec = it2.Get();
        DisplacementFieldType::PixelType vec = field->GetPixel(ind3);
...
{   
    ...
    CurrentImageType::Pointer resampled_smoothed_fixed_TRCC=nullptr;
    CurrentImageType::Pointer resampled_smoothed_moving_TRCC=nullptr;
    ...
    for(int met=0; met< Nmetrics;met++)
    {
        ...
        if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::TRCC)
        {
            resampled_smoothed_fixed_TRCC=CUDAIMAGE::New();
            resampled_smoothed_fixed_TRCC->DuplicateFromCUDAImage(this->settings->metrics[met].fixed_img);
            resampled_smoothed_moving_TRCC=CUDAIMAGE::New();
            resampled_smoothed_moving_TRCC->DuplicateFromCUDAImage(this->settings->metrics[met].moving_img);

            if(this->settings->img_smoothing_std!=0)
            {
                resampled_smoothed_fixed_TRCC= GaussianSmoothImage(resampled_smoothed_fixed_TRCC,this->settings->img_smoothing_std*this->settings->img_smoothing_std);
                resampled_smoothed_moving_TRCC= GaussianSmoothImage(resampled_smoothed_moving_TRCC,this->settings->img_smoothing_std*this->settings->img_smoothing_std);
            }

            if(resampled_smoothed_fixed_TRCC->sz.x != this->virtual_img->sz.x)
            {
                resampled_smoothed_fixed_TRCC=ResampleImage(resampled_smoothed_fixed_TRCC,this->virtual_img);
                resampled_smoothed_moving_TRCC=ResampleImage(resampled_smoothed_moving_TRCC,this->virtual_img);
            }

            resampled_smoothed_fixed_TRCC->CreateTexture();
            resampled_smoothed_moving_TRCC->CreateTexture();
        }
        ...
        if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::DTDEV)
        {
           ...
           // CurrentImageType::Pointer log_moving_tensor= LogTensor(this->settings->metrics[met].moving_img);
            ...
            {                                
                ...
                {
                    if(v==1 || v==2 || v==4) // a bit smooth off-diagonals
                    {
                        log_fixed_tensor_vec[v]= GaussianSmoothImage(log_fixed_tensor_vec[v],0.25);
                        log_moving_tensor_vec[v]= GaussianSmoothImage(log_moving_tensor_vec[v],0.25);
                    }
                }
    ...
    while( iter++ < this->settings->niter && !converged )
    {
        ...
        CurrentFieldType::Pointer warpFieldFINV=this->def_FINV;
        CurrentFieldType::Pointer warpFieldMINV=this->def_MINV;
        if(this->settings->const_init_finv)
        {
            warpFieldFINV=ComposeFields(this->def_FINV,this->settings->const_init_finv);
            warpFieldMINV=ComposeFields(this->def_MINV,this->settings->const_init_minv);
        }


        CUDAIMAGE::Pointer TR_img_fixed=nullptr,TR_img_moving=nullptr;
        CUDAIMAGE::Pointer FA_img_fixed=nullptr,FA_img_moving=nullptr;
        ...
        for(int met=0; met< Nmetrics;met++)
        {
            ...
            if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::DTCC)
            {
                ...
                CurrentImageType::Pointer warped_fixed_img = WarpImage(resampled_smoothed_fixed_structurals[str_id],warpFieldFINV);
                CurrentImageType::Pointer warped_moving_img = WarpImage(resampled_smoothed_moving_structurals[str_id],warpFieldMINV);
                str_id++;
            ...
            if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::DTTR)
            {
                CurrentImageType::Pointer warped_fixed_img = WarpImage(resampled_smoothed_fixed_TR,warpFieldFINV);
                CurrentImageType::Pointer warped_moving_img = WarpImage(resampled_smoothed_moving_TR,warpFieldMINV);

                TR_img_fixed=CUDAIMAGE::New();
                TR_img_fixed->DuplicateFromCUDAImage(warped_fixed_img);
                TR_img_moving=CUDAIMAGE::New();
                TR_img_moving->DuplicateFromCUDAImage(warped_moving_img);

                metric_value = ComputeMetric_MSQ(warped_fixed_img,warped_moving_img, updateFieldF_temp,updateFieldM_temp );
            }
            if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::TRCC)
            {
                CurrentImageType::Pointer warped_fixed_img = WarpImage(resampled_smoothed_fixed_TRCC,warpFieldFINV);
                CurrentImageType::Pointer warped_moving_img = WarpImage(resampled_smoothed_moving_TRCC,warpFieldMINV);

                metric_value =ComputeMetric_CC(warped_fixed_img,warped_moving_img, updateFieldF_temp,updateFieldM_temp );
            }
            if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::DTFA)
            {
                CurrentImageType::Pointer warped_fixed_img = WarpImage(resampled_smoothed_fixed_FA,warpFieldFINV);
                CurrentImageType::Pointer warped_moving_img = WarpImage(resampled_smoothed_moving_FA,warpFieldMINV);

                FA_img_fixed=CUDAIMAGE::New();
                FA_img_fixed->DuplicateFromCUDAImage(warped_fixed_img);
                FA_img_moving=CUDAIMAGE::New();
                FA_img_moving->DuplicateFromCUDAImage(warped_moving_img);
            ...
            if(this->settings->metrics[met].MetricType== DRTAMASMetricEnumeration::DTDEV)
            {
                ...
                for(int v=0;v<log_fixed_tensor_vec.size();v++)
                {
                    CurrentImageType::Pointer ft= WarpImage(log_fixed_tensor_vec[v],warpFieldFINV);
                    CurrentImageType::Pointer mt= WarpImage(log_moving_tensor_vec[v],warpFieldMINV);
...
/*
      ...
      //  std::cout<<updateFieldFCPU2->GetPixel(ind3) - updateFieldFCPU->GetPixel(ind3)<<std::endl;
        ...
        ScaleUpdateField(updateFieldM2,this->settings->learning_rate);
...
*/
        ...
        this->def_FINV  = InvertField(this->def_F,this->def_FINV);
        this->def_MINV  = InvertField(this->def_M,this->def_MINV);

        if(this->settings->constrain)
        {
            ContrainDefFields(this->def_FINV,this->def_MINV);
        }

        this->def_F  = InvertField(this->def_FINV,this->def_F);
        this->def_M  = InvertField(this->def_MINV,this->def_M);
        ...
        {
            if(this->settings->downsample_factor==2 && iter>=10)
                converged = true;

            if(this->settings->downsample_factor==1 && iter>=20)
                converged = true;
    ...
    if(this->settings->const_init_finv)
    {
        this->settings->output_finv= ComposeFields(this->def_FINV,this->settings->const_init_finv);
        this->settings->output_minv= ComposeFields(this->def_MINV,this->settings->const_init_minv);
    }
    else
    {

...
int main( int argc , char * argv[] )
{
    if(argc<2)    
    {
        std::cout<< "Computes the Radial Diffusivity map from the diffusion tensor"<<std::endl;
        std::cout<<"Usage:   ComputeRDMap full_path_to_tensor_image"<<std::endl;
        return 0;
    }
    ...
    for(int k=0;k<imsize[2];k++)
    {
        ...
        for(int j=0;j<imsize[1];j++)
        {
            ...
            for(int i=0;i<imsize[0];i++)
            {
                ...
                double wl = (eig.D(0,0)+eig.D(1,1))/2.;
    ...
    std::string output_name=currdir + basename + std::string("_RD.nii");

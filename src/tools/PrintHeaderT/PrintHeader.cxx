...
//  std::cout << " Orientation " << reader->GetOutput()->GetOrientation() << std::endl;
  ...
  bb1.set_size(VDim);
  ospm.set_size(VDim);
  itk::ContinuousIndex<double,ImageDimension> bound_start_index, bound_end_index;
  typename ImageType::PointType bound_start_point,bound_end_point;
  ...
  {
      vnl_vector<double> ind(VDim);
      ind.fill(0);
      ind[i]=1;

      vnl_vector<double> out = image->GetDirection().GetVnlMatrix() *ind;

      if( (fabs(out[0]) >  fabs(out[1]))  && (fabs(out[0]) >  fabs(out[2])))
      {
          if(out[0]<0)
              mult[i]=-1;
      }

      if( (fabs(out[1]) >  fabs(out[0]))  && (fabs(out[1]) >  fabs(out[2])))
      {
          if(out[1]<0)
              mult[i]=-1;
      }

      if( (fabs(out[2]) >  fabs(out[0]))  && (fabs(out[2]) >  fabs(out[1])))
      {
          if(out[2]<0)
              mult[i]=-1;
      }
  ...
  bound_end_index[0]=image->GetLargestPossibleRegion().GetSize()[0] -1+ 0.5;
  bound_end_index[1]=image->GetLargestPossibleRegion().GetSize()[1] -1+ 0.5;
  bound_end_index[2]=image->GetLargestPossibleRegion().GetSize()[2] -1+ 0.5;


  image->TransformContinuousIndexToPhysicalPoint(bound_start_index,bound_start_point);
  image->TransformContinuousIndexToPhysicalPoint(bound_end_index,bound_end_point);
...
private:
  ...
  itk::ImageIOBase::Pointer imageIO =
    itk::ImageIOFactory::CreateImageIO(
      fn.c_str(), itk::ImageIOFactory::IOFileModeEnum::ReadMode);

...
int main( int argc , char * argv[] )
{
    if(argc<2)    
    {
        std::cout<<"Usage:   ComputeTRMap full_path_to_tensor_image"<<std::endl;
        return 0;
    }
    ...
    for(int k=0;k<imsize[2];k++)
    {
        ...
        for(int j=0;j<imsize[1];j++)
        {
          ...
          //      if(curr_tens(0,0)+curr_tens(1,1)+curr_tens(2,2) > 10)
            //        int mama=0;
    ...
    std::string output_name=currdir + basename + std::string("_TR.nii");

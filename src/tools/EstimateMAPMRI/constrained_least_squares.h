            ...
            Problem (const matrix_type& problem_matrix, 
              ...
              max_niter (max_iterations ? max_iterations : 10*problem_matrix.cols()) {
                ...
                if (H.cols() != constraint_matrix.cols())
                  std::cout<<"FIXME: dimensions of problem and constraint problem do not match (ICLS)"<<std::endl;

                if (solution_min_norm_regularisation < 0.0)
                  std::cout<<"FIXME: solution norm regularisation is negative (ICLS)"<<std::endl;

                if (lambda_min_norm < 0.0)
                  std::cout<<"FIXME: constraint norm regularisation is negative (ICLS)"<<std::endl;

                if (tolerance < 0.0)
                  std::cout<<"FIXME: tolerance is negative (ICLS)"<<std::endl;

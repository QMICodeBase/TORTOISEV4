...
int main( int argc , char * argv[] )
{
    if(argc<5)    
    {
        std::cout<<"Usage:   CombineTransformationsWithOutputName  -out full_path_to_output_trans_name -trans full_path_to_trans1 full_path_to_trans2 ..............   full_path_to_trans_n "<<std::endl;
        return 0;
    }
    
    int out_tag_pos=-1;
    int trans_tag_pos=-1;
    int last_trans_pos=-1;
    ...
    for(int i=0;i<argc;i++)
    {
        ...
        if(string(argv[i])==string("-trans"))
            trans_tag_pos=i;
    }
    
    if(out_tag_pos==-1)
    {
        std::cout<<"-out tag not present in the arguments..."<<std::endl;
        std::cout<<"Usage:   CombineTransformationsWithOutputName  -out full_path_to_output_trans_name -trans full_path_to_trans1 full_path_to_trans2 ..............   full_path_to_trans_n "<<std::endl;
        return 0;        
    }
    ...
    if(trans_tag_pos==-1)
    {
        std::cout<<"-trans tag not present in the arguments..."<<std::endl;
        std::cout<<"Usage:   CombineTransformationsWithOutputName  -out full_path_to_output_trans_name -trans full_path_to_trans1 full_path_to_trans2 ..............   full_path_to_trans_n "<<std::endl;
        return 0;        
    }
    
    if(out_tag_pos<trans_tag_pos)
    {
        last_trans_pos=argc-1;        
    }
    else
    {
        last_trans_pos= out_tag_pos-1;
    }
    ...
    std::string currdir;
    std::string nm(argv[out_tag_pos+1]);
    ...
    bool all_affine=1;    
    for(int i=trans_tag_pos+1;i<=last_trans_pos;i++) 
    {
        ...
        if(idx != std::string::npos)
        {
            ...
            else
            {
                ...
                {
                    std::cout<<"Transformation " << i-trans_tag_pos << " does not contain .nii or .txt.. Exiting!!"<<std::endl;
                    exit(0);                                    
                }                
            }
        }

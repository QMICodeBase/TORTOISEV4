...
void FINALDATA::Generate()
{
    ...
    for(int PE=0;PE<2;PE++)
    {
        ...
        std::vector<ImageType3D::Pointer> final_data,raw_data, native_native_synth_vols;
        std::vector<ImageType3DBool::Pointer> native_inclusion_imgs,final_inclusion_imgs;
        final_data.resize(nvols);
        raw_data.resize(nvols);
        native_inclusion_imgs.resize(nvols);
        final_inclusion_imgs.resize(nvols);
        native_native_synth_vols.resize(nvols);

        for(int vol=0;vol<nvols;vol++)
            raw_data[vol]= read_3D_volume_from_4D(data_names[PE],vol);


        (*stream)<<"Volume done: "<<std::flush;
        ...
        for(int vol=0;vol<nvols;vol++)
        {
            ...
            final_data[vol]=ImageType3D::New();
            final_data[vol]->SetRegions(ref_img->GetLargestPossibleRegion());
            final_data[vol]->Allocate();
            final_data[vol]->SetSpacing(ref_img->GetSpacing());
            final_data[vol]->SetDirection(ref_img->GetDirection());
            final_data[vol]->SetOrigin(ref_img->GetOrigin());
            ...
            ImageType3D::SizeType final_sz= final_data[vol]->GetLargestPossibleRegion().GetSize();
            ...
            for(it.GoToBegin();!it.IsAtEnd();++it)
            {
                ImageType3D::IndexType ind3=it.GetIndex();
                ImageType3D::PointType pt,pt_trans;
                final_data[vol]->TransformIndexToPhysicalPoint(ind3,pt);
            ...
            if(final_inclusion_imgs.size()==0)
            {
                #pragma omp critical
                {
        ...
        vnl_matrix<double> rot_Bmat= RotateBMatrix(Bmatrix,this->dwi_transforms[PE],id_trans);
        rot_Bmat= RotateBMatrix(rot_Bmat,this->b0_t0_str_trans->GetMatrix().GetVnlMatrix(),final_data[0]->GetDirection().GetVnlMatrix());
        ...
        if(final_inclusion_imgs.size())
        {
            ...
            float dti_bval_cutoff= RegistrationSettings::get().getValue<float>(std::string("high_b_dti_end_bval"));
            float mapmri_bval_cutoff= RegistrationSettings::get().getValue<float>(std::string("high_b_mapmri_start_bval"));
            ...
            // MAPMRI FITTING                       
            float small_delta= this->jsons[PE]["SmallDelta"];
            float big_delta= this->jsons[PE]["BigDelta"];
            ...
            std::vector<ImageType3D::Pointer> synth_imgs_dt;
            std::vector<ImageType3D::Pointer> synth_imgs_map;
            synth_imgs_dt.resize(Nvols[PE]);
            synth_imgs_map.resize(Nvols[PE]);
            ...
            for(int vol=0;vol<Nvols[PE];vol++)
            {
                ...
                synth_imgs_dt[vol]= dti_estimator.SynthesizeDWI( rot_Bmat.get_row(vol) );
                if(MAPMRI_indices.size()>0)
                    synth_imgs_map[vol] = mapmri_estimator.SynthesizeDWI( rot_Bmat.get_row(vol));
                else
                    synth_imgs_map[vol] =nullptr;
            ...
            for(int vol=0;vol<Nvols[PE];vol++)
            {
                ...
                if(synth_imgs_map[vol])
                    synth_img=synth_imgs_map[vol];
                else
                    synth_img=synth_imgs_dt[vol];
                ...
                filter->SetInput2(final_mask);
                filter->SetInput1(final_inclusion_imgs[vol]);
                filter->Update();
                ImageType3D::Pointer weight_img= filter->GetOutput();

                //Check if we have a valid voxel
                ImageType3D::Pointer final_data2=final_data[vol];
                bool allzeros=true;
                ...
                // Perform B1 inhomogeneity correction if image is valid
                if(!allzeros)
                {
                    ...
                    n4_filter->SetMaskImage(weight_img);
                    n4_filter->Update();
                    final_data2 = n4_filter->GetOutput();
                }
                ...
                for(int k=0;k<sz[2];k++)
                {
                    ...
                    //Fit a quadratic spatial scaling function using only these intersection voxels
                        ...
                        for(int i=0;i<sz[0];i++)
                        {
                            ...
                            if(final_inclusion_imgs[vol]->GetPixel(ind3)==0)
                            {
                                ...
                                else
                                {
                                    ...
                                    if(synth_imgs_map[vol])
                                        val=synth_imgs_map[vol]->GetPixel(ind3);
                                    else
                                        val=synth_imgs_dt[vol]->GetPixel(ind3);

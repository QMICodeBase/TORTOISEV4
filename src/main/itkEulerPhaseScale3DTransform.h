...
namespace itk
{
/** \class EulerPhaseScale3DTransform
 *
 * \brief EulerPhaseScale3DTransform of a vector space (e.g. space coordinates)
 *
...
class ITK_TEMPLATE_EXPORT EulerPhaseScale3DTransform : public Rigid3DTransform<TParametersValueType>
{
public:
  ITK_DISALLOW_COPY_AND_MOVE(EulerPhaseScale3DTransform);

  /** Standard class type aliases. */
  using Self = EulerPhaseScale3DTransform;
  ...
  itkOverrideGetNameOfClassMacro(EulerPhaseScale3DTransform);
  ...
  void SetPhaseScale(double ps){this->m_PhaseScale=ps;}
  /** @ITKEndGrouping */
...
protected:
  EulerPhaseScale3DTransform(const MatrixType & matrix, const OutputPointType & offset);
  EulerPhaseScale3DTransform(unsigned int parametersDimension);
  EulerPhaseScale3DTransform();

  ~EulerPhaseScale3DTransform() override = default;
...
#  include "itkEulerPhaseScale3DTransform.hxx"
#endif

#endif /* itkEulerPhaseScale3DTransform_h */

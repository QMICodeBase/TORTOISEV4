...
vnl_matrix<int> DIFFPREP::ParseJSONForSliceTiming(json cjson)
{
    ...
    std::sort(slice_timings.begin(), slice_timings.end()); // Sort the vector
    auto last = std::unique(slice_timings.begin(), slice_timings.end()); // Move unique elements to the front
    slice_timings.erase(last, slice_timings.end()); // Erase the duplicate elements

    int N_unique_times= slice_timings.size();
    int MB=0;
    for(int s=0;s<slice_timings_orig.size();s++)
    {
    ...
    for(int sg=0;sg<slice_timings.size();sg++)
    {
        int col=0;
        for(int s=0;s<slice_timings_orig.size();s++)
        {
